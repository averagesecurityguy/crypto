...
Function: open_file
-------------------
Attempt to open a file. If there is an error write it to stderr and return NULL.
*/
...
*/
int generate_key_nonce(unsigned char *key, unsigned char *nonce, char *passphrase)
{
    ...
    if (crypto_pwhash_scryptsalsa208sha256
        (key, KEYBYTES, passphrase, strlen(passphrase), (unsigned char *)key_salt,
    ...
    if (crypto_pwhash_scryptsalsa208sha256
        (nonce, NONCEBYTES, passphrase, strlen(passphrase), (unsigned char *)nonce_salt,
        crypto_pwhash_scryptsalsa208sha256_OPSLIMIT_INTERACTIVE,
        crypto_pwhash_scryptsalsa208sha256_MEMLIMIT_INTERACTIVE) != 0)
    {
...
*/
int encrypt_file(char *pfname, char *efname, unsigned char *k, unsigned char *n)
{
    size_t rsize;
...
*/
int decrypt_file(char *efname, char *pfname, unsigned char *k, unsigned char *n)
{
    size_t rsize;
    unsigned char ebuf[BUFBYTES + MACBYTES];
    unsigned char pbuf[BUFBYTES];
...
*/
void get_key_material(unsigned char *key, unsigned char *nonce, char *passphrase)
{
...
*/
void process_file(char *command, char *infile, char *outfile, unsigned char *key, unsigned char *nonce)
{
    int result = 0;
